...
（与 server 侧对齐）。装配期对尾 `/*` 条目打聚合 WARN，但**两个条件同时成立才打**
（v0.1.23 起），否则静默：
...
auth:
  anonymous_paths:
    - /auth/login
    - { path: "/idp/*", one_layer: true }   # 明知 ** 更宽，就要一层
```

> `one_layer` 只对尾 `/*` 条目有意义：标在别的条目上装配期直接报错。
...
| 启动 WARN 反复点名某条 `/*` | 该条改写为 `**` 真会多命中已注册路由——按提示改 `**`；确属有意一层就标 `{ path: …, one_layer: true }` 消音 |
| 装配期报 `标了 one_layer，但没有尾 "/*"` | 标记只对尾 `/*` 条目有意义，去掉或补上通配 |
