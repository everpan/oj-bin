...
- [ ] 结构体 + 字段 + Default `static_sites: Vec::new()`
- [ ] 测试：`static_sites` YAML round-trip（两条：prefix `/docs` path `d1`、prefix `/` path `d2`）；缺省 `ServerCfg::default().static_sites.is_empty()`
- [ ] `cargo test --release static_sites`（根 crate）+ commit
...
- [ ] args.rs：`app_path: Option<String>` → `Vec<String>`；测试 `server_defaults_and_overrides` 适配（单值 → `vec!["web"]`；新增多值用例 `--app-path a --app-path /x=b` → `["a", "/x=b"]`）
- [ ] server_cmd.rs：run() 改用 fold（裸多值 Err `"--app-path <dir> (bare) may appear at most once"`；prefix 过 `resolve_app_prefix`）；admission_gate 改 `app_specified: bool`（存在性检查删除，错误文案保留 api 部分）
- [ ] server_cmd tests：gate 新签名全适配；fold：裸单值/裸多值报错/prefix=dir 追加+同前缀替换/裸+带前缀混合
...
- [ ] AppState 字段替换 + 静态兜底分支 `find_map`（最长命中；命中站点 miss → 该站 spa 回落，不跨站）
- [ ] 三构造器签名替换 + lib.rs 内全部测试适配（spawn_static 辅助等）
- [ ] 新测试：双站点最长前缀（`/docs/api/x` 胜过 `/docs`）；`/` 兜底 catch-all；站内 miss 不跨站（A 站 miss 不回落 B 站）；per-site meta（仅 A 站有 `__meta` 目录）；spa_fallback 每站独立
- [ ] `cargo test --release -p server` + commit
